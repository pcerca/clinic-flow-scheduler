...
import { setupPin, verifyPin, isPinSet, saveSecureData, loadSecureData } from './utils/storage';
...
function App() {
  ...
  const [isFirstRun, setIsFirstRun] = useState(!isPinSet());
  ...
  useEffect(() => {
    ...
    const saved = loadSecureData('clinic_patients', pin);
    const savedHolidays = loadSecureData('clinic_holidays', pin);
    ...
    if (saved) {
      try {
        let parsed = saved;
        if (typeof saved === 'string') parsed = JSON.parse(saved);
      ...
      if (isFirstRun) {
        const oldPlaintext = localStorage.getItem('clinic_patients');
        if (oldPlaintext) {
          try {
  ...
  useEffect(() => {
    if (isAuthenticated && pin && patients.length > 0) {
      saveSecureData('clinic_patients', patients, pin);
    }
  }, [patients, isAuthenticated, pin]);

  useEffect(() => {
    if (isAuthenticated && pin && Object.keys(holidays).length > 0) {
      saveSecureData('clinic_holidays', holidays, pin);
    }
  }, [holidays, isAuthenticated, pin]);
  ...
  if (!isAuthenticated) {
    return <LoginScreen isSetup={isFirstRun} onLogin={handleLogin} error={authError} />;
  }
