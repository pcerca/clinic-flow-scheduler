      ...
      <div className="space-y-3">
        {dailySessions.length === 0 ? (
          ...
          dailySessions.map(({ patient, session }) => {
            ...
            <div 
              key={session.id} 
              className={`relative flex flex-col md:flex-row md:items-center justify-between p-4 rounded-2xl border transition-all duration-300 gap-4 group ${
                session.status === SessionStatus.ABSENT ? 'bg-slate-50 border-slate-200 opacity-75 grayscale' :
                session.isLocked ? 'bg-violet-50/50 border-violet-200 shadow-sm' :
                session.status === SessionStatus.COMPLETED ? 'bg-emerald-50/50 border-emerald-200 shadow-sm' :
                'bg-white border-transparent shadow-card hover:shadow-lg hover:-translate-y-0.5 hover:border-primary/20'
              }`}
            >
              {/* Main Content */}
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-2">
                    ...
                    <div className={`px-2.5 py-1 rounded-lg text-xs font-bold shadow-sm ${
                        session.status === SessionStatus.ABSENT ? 'bg-slate-200 text-slate-500' : 
                        session.status === SessionStatus.COMPLETED ? 'bg-emerald-100 text-emerald-700' :
                        'bg-white border border-primary/20 text-primaryDark'
                    }`}>
                        {session.time || '00:00'}
                    </div>
                ...
                <h4 className={`font-bold text-lg truncate ${
                    session.status === SessionStatus.ABSENT ? 'text-slate-500 line-through' : 
                    session.status === SessionStatus.COMPLETED ? 'text-emerald-900' :
                    session.isLocked ? 'text-violet-900' : 
                    'text-slate-800'
                }`}>
                  {patient.name}
                </h4>
                ...
                {!session.isLocked && (
                  <>
                    <button
                        ...
                        session.status === SessionStatus.ABSENT ? 'text-slate-300 cursor-not-allowed' :
