...
import { CalendarPlus, Home, Building, Clock, Save, Lock, RefreshCw, Loader2 } from 'lucide-react';
...
export const AddPatientForm: React.FC<AddPatientFormProps> = ({ initialData, onSave, onClose, lang }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Auto-geocode address for domicile patients if no coordinates yet
    let finalCoordinates = coordinates;
    if (location === LocationType.HOME && address && !coordinates) {
      setIsGeocoding(true);
      try {
    ...
    if (initialData) {
      const updatedPatient: Patient = {
        ...
        nomenclature,
        location,
        address: location === LocationType.HOME ? address : undefined,
        coordinates: location === LocationType.HOME ? finalCoordinates : undefined,
        startConfig,
      };
      ...
      {location === LocationType.HOME && (
        <div>
          ...
          <textarea
            ...
            placeholder="e.g. 123 Main Street, Apartment 4B, City"
            rows={3}
          />
          <div className="flex items-center gap-2 mt-1">
            {isGeocoding && <Loader2 className="w-3 h-3 animate-spin text-primary" />}
            <p className="text-xs text-slate-400">
              {isGeocoding ? 'Locating address...' : "Address will be automatically located on the map when you save."}
            </p>
          </div>
        </div>
      )}
            ...
            return (
              <div key={day.id} className="flex items-center gap-3">
                <button
                  ...
                  className={`w-10 h-10 shrink-0 rounded-xl flex items-center justify-center text-xs font-bold transition-all duration-200 ${isSelected
                ...
                <div className="flex flex-wrap gap-2">
                  {[1, 2, 3, 6, 12].map(m => (
                    <button
                      ...
                      className={`px-3 py-1.5 text-xs font-bold rounded-lg border transition-all ${durationMonths === m
