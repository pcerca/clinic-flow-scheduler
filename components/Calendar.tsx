...
import { ChevronLeft, ChevronRight, CalendarDays, CalendarRange, LayoutList, Building, Home, User, CheckCircle2 } from 'lucide-react';
        ...
        <div className="flex flex-col h-full">
            ...
            <div className="grid grid-cols-7 auto-rows-fr flex-1 bg-slate-100 gap-px border-slate-200 overflow-y-auto touch-pan-y p-1 rounded-b-2xl">
                ...
                return (
                    ...
                    className={`min-h-[80px] p-2 relative group hover:bg-primary/5 hover:z-10 hover:shadow-md transition-all cursor-pointer rounded-lg m-[1px] ${
                        isToday ? 'bg-white ring-2 ring-primary ring-inset z-10' : 
                                ...
                                {sessions.slice(0, 8).map((s, idx) => (
                                    <div 
                                        ...
                                        className={`w-1.5 h-1.5 rounded-full transition-colors ${
                                            s.status === SessionStatus.ABSENT ? 'bg-red-400' :
        ...
        <div className="flex flex-col h-full">
            ...
            <div className="grid grid-cols-7 flex-1 bg-slate-50 divide-x divide-slate-200 overflow-y-auto p-2 gap-2">
                {days.map(day => {
                    ...
                    return (
                        <div key={dateISO} onClick={() => onDayClick(dateISO)} className={`flex flex-col rounded-xl transition-all cursor-pointer min-h-[200px] ${
                            isToday ? 'bg-white ring-1 ring-primary/20 shadow-lg shadow-primary/5' : 
                                ...
                                {sortedTimes.map(time => (
                                    <div key={time} className="flex flex-wrap gap-1">
                                        {sessionsByTime[time].map(({session, patient, status, isLocked}) => (
                                            <div key={session.id} className={`flex-1 min-w-[45%] p-2 rounded-lg border text-xs transition-all hover:scale-105 ${
                                                status === SessionStatus.ABSENT ? 'bg-slate-50 border-slate-100 text-slate-400 line-through opacity-70' :
                                                isLocked ? 'bg-violet-50 border-violet-100 text-violet-700' :
                                                status === SessionStatus.COMPLETED ? 'bg-emerald-50 border-emerald-100 text-emerald-700' :
                                                'bg-white border-primary/10 text-slate-700 shadow-sm hover:border-primary/40'
                                            }`}>
                                                <div className="font-bold flex justify-between items-center mb-0.5">
                                                    <span className={status === 'SCHEDULED' ? 'text-primaryDark' : ''}>{session.time}</span>
                                                    {patient.location === 'HOME' ? <Home className="w-2.5 h-2.5 opacity-40 shrink-0"/> : <Building className="w-2.5 h-2.5 opacity-40 shrink-0"/>}
                                                </div>
                 ...
                 <div className="divide-y divide-slate-50 overflow-y-auto flex-1 bg-white">
                    {unvalidated.length === 0 ? (
                        ...
                        unvalidated.map(session => {
                            ...
                            return (
                            <div key={session.id} className="p-4 hover:bg-slate-50 flex items-center justify-between group transition-all duration-200 border-l-4 border-transparent hover:border-primary">
                                <div className="flex items-center gap-6">
                                    ...
                                    <div>
                                        <div className="flex items-center gap-2 mb-1.5">
                                            <span className="font-mono font-bold text-primaryDark bg-primary/10 px-2.5 py-0.5 rounded-lg text-xs">
                                                {session.time}
                                            </span>
